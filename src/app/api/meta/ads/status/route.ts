/**
 * Ativa ou pausa um anúncio no Meta Ads.
 * POST /api/meta/ads/status
...
export async function POST(req: Request) {
  try {
    ...
    if (!ad_id || !status) {
      return NextResponse.json(
        { error: "ad_id e status são obrigatórios." },
        { status: 400 }
      );
    }
    ...
    if (json.error && !json.success) {
      return NextResponse.json(
        { error: json.error?.message ?? "Erro ao atualizar status do anúncio.", meta_error: json.error },
        { status: 500 }
      );
    }
